...
void CMyApp::Render()
{
	...
	{
		parallel_proj_x_mat[0][1] = -m_camera.GetAt().y / m_camera.GetAt().x;
		parallel_proj_x_mat[0][2] = -m_camera.GetAt().z / m_camera.GetAt().x;
		suzanne1World *= glm::transpose(parallel_proj_x_mat);

	}
	else if (parallerl_proj_mode && e == 1)
	{
		parallel_proj_x_mat[1][0] = -m_camera.GetAt().x / m_camera.GetAt().y;
		parallel_proj_x_mat[1][2] = -m_camera.GetAt().z / m_camera.GetAt().y;
		suzanne1World *= glm::transpose(parallel_proj_y_mat);
	}
	else if (parallerl_proj_mode && e == 2)
	{
		parallel_proj_x_mat[2][1] = -m_camera.GetAt().x / m_camera.GetAt().z;
		parallel_proj_x_mat[2][2] = -m_camera.GetAt().y / m_camera.GetAt().z;
		suzanne1World *= glm::transpose(parallel_proj_z_mat);
	}
